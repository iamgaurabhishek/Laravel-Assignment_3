...
<div class="row">
    <div class="col">
        <h1 class="display-2">Update Client Profile</h1>
    </div>
</div>
<div class="row">
    <form action="{{ route('clients.update', $client->id) }}" method="PUT">
        ...
        @endif{{ csrf_field() }}<div class="mb-3"><label for="name" class="form-label">
            Name
                ...
                aria-describedby="name" value="{{ $client->name }}"></div>
        <div class="mb-3"><label for="phone" class="form-label">Phone</label><input type="text"
                class="form-control" id="phone" name="phone" aria-describedby="phone"
